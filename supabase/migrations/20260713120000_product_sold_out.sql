-- ─────────────────────────────────────────────────────────────────────────────
-- "Sold Out" MANUAL: el admin puede marcar una prenda como agotada aunque tenga
-- stock. products.sold_out = true → la tienda muestra la cinta "SOLD OUT" y
-- bloquea la compra (además del agotado automático por stock = 0).
-- create_product acepta payload.sold_out. IDEMPOTENTE.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  insert into public.products (
    ...
    is_active, sold_out, category, categories, tipo_prenda, badge, is_launch, launch_order
  ) values (
    v_name,
    ...
    coalesce((payload->>'sold_out')::boolean, false),
