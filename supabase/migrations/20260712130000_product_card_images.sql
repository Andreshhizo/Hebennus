-- ─────────────────────────────────────────────────────────────────────────────
-- Fotos de TARJETA independientes de la galería de detalle.
--   products.card_images = [portada, hover]  → SOLO la tarjeta del catálogo.
--   products.images       = galería de la ficha (detalle) → SOLO la ficha.
-- Antes ambas convivían en `images` (portada+hover+galería) y se "mezclaban".
-- create_product acepta payload.card_images. IDEMPOTENTE.
-- ─────────────────────────────────────────────────────────────────────────────

begin;

alter table public.products add column if not exists card_images text[] not null default '{}';
...
begin
  ...
  insert into public.products (
    name, slug, description, price, images, card_images, images_by_color,
    ...
    coalesce((select array_agg(u) from jsonb_array_elements_text(coalesce(payload->'card_images', '[]'::jsonb)) as u), '{}'::text[]),
